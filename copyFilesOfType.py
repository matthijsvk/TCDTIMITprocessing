...
def copyFilesOfType(srcDir, dstDir, extension):
    ...
    for root, dirs, files in os.walk(srcDir):
        for file_ in files:
            if file_.endswith(extension):
                ...
                relSrcPath = relpath(srcDir, srcPath)
                destPath = os.path.join(dstDir, relSrcPath)
                print("copying from : %s to \t\t %s" % (srcPath, destPath))
                src.append(srcPath)
                dest.append(destPath)

    print(len(src), " files will be copied")
    if query_yes_no("Are you sure you want to peform these operations?","yes"):
...
if __name__ == '__main__':
    ...
    copyFilesOfType(srcDir,dstDir,extension)


# example usage: python3 fileDirOps.py ~/Documents/Dropbox/_MyDocs/_ku_leuven/Master_2/Thesis/convNets/code /media/matthijs/TOSHIBA_EXT/TCDTIMIT/zzzNPZmodels ".npz"
